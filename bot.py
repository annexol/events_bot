            ...
            if 10 < int(event.text) < 100:
                ...
                if len(person) >= 4:
                    ...
                    if status == 'add':
                        ...
                        bot.send_message(event.from_user.id, text='Вы уже есть в списке...', reply_markup=show_people)
                else:
...
'''function for working with pagination'''
...
def answer_paginator(call):
    ...
    text = ''  # formation of text messages
    ...
    bot.send_message(call.from_user.id, text=text, parse_mode='MarkdownV2',
...
def answer(call):  # user gender detection
    if call.data == 'Парень' or call.data == 'Девушка':
        ...
        show = DataBase()
        show.connection()
        persons = show.show_people()
        text = 'Люди которые ищут компанию:' + '\n' + '\n'
        for i in persons:
            text += '💥{}: {}, {} {}, написать:@{}'.format(str(i[3]), str(i[1]),
                                                           str(i[4]), 'лет', str(i[2]) + '\n' + '\n')
